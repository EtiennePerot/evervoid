            ...
            public void run()
            {
                ...
                try {
                    ...
                    LoggerUtils.warning("Caught exception " + e + "with message " + e.getMessage()
                                    + ".  While trying to send a load game.");
                    if (onFailure != null) {
    ...
    private void guiMessageReceived(final EVMessage message)
    {
        ...
        if (messageType.equals(GameStateMessage.class.getName())) {
            try {
                ...
                Logger.getLogger(EverVoidClient.class.getName()).log(Level.SEVERE,
                                "The Server has sent a bad game state", e);
            }
            ...
            if (aInLobby) {
                for (final EVLobbyMessageListener observer : aLobbyObservers) {
                    observer.serverDied();
                        ...
                        || messageType.equals(ServerChatMessage.class.getName())) {
            for (final EVGlobalMessageListener observer : aGlobalObservers) {
                observer.receivedChat(messageContents.getStringAttribute("player"),
                                new Color(messageContents.getAttribute("color")),
