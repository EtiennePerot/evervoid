...
/**
 * An Action is anything that a user could do in game that would affect the State. Every Action has an acting
 * {@link Player}, to
 * determined who created the Action. The Action also has an {@link EVGameState}, which it will use to determine its
 * validity
 * and on which it will execute itself. Actions are carried out by calling the execute() function. This function first
 * checks
 * for validity by calling isValid(), then executes by calling executeAction(). Subclasses of Action should override
 * isValidAction() and executeAction() as the isValid() and execute() calls do some logic before calling these
 * functions.
...
{
    /**
     * Creates an Action from the Json object passed. The particular type of Action created is determined by the
     * actiontype
     * attribute. This attribute must be a string in the form of a Class name (ex.
     * com.evervoid.state.action.ship.moveship) and
     ...
     *             If the state and json arguments are not instance of EVGameState and Json respectively. (This should
     *             never be
     *             thrown)
     * @throws IllegalEVActionException
     *             The action may throw this exception upon construction if it is not valid.
     */
    public static Action deserializeAction(final EVGameState state, final Json json) throws ClassNotFoundException,
                    ...
                    IllegalAccessException, NoSuchMethodException, IllegalEVActionException
    {
    ...
    public Action clone()
    {
        try {
            ...
            LoggerUtils.severe("Coning Action failed, something went horribly wrong");
            e.printStackTrace();
    ...
    /**
     * Check if this Action is valid. Calls the template method isValidAction iff action player is valid in the first
     * place.
