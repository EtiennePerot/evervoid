...
cp "$src/version.txt" "$builddir/build/.version"
...
popd &>/dev/null
echo "Cleaning environment."
mv "$builddir/everVoid.jar" "$package"
rm -rf "$builddir"
echo "JAR build complete. JAR file is available at $package"
